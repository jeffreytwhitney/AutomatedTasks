...
def CopyFolderToSubDirectoryWithFileNameAppend(source_directory, sub_directory, ini_file_name, show_messagebox=True):
    ...
    for file in os.scandir(source_directory):
        if file.is_file():
            FileArchiveLib.CopyFileWithFileNameAppend(
                current_filename=file.path, output_directory=output_subdirectory, show_messagebox=False)
    if show_messagebox:
...
def CopyFolderWithFileNameAppend(source_directory, ini_file_name, show_messagebox=True, recurse=False):
    output_directory = ArchiveLib.GenerateFolderOutputPath(
        source_directory, ini_file_name
    )
    if output_directory == "":
        return
    for item in os.scandir(source_directory):
        if item.is_file():
            FileArchiveLib.CopyFileWithFileNameAppend(current_filename=item.path, output_directory=output_directory,
...
def CopyFolderWithOverwrite(source_directory, ini_file_name, show_messagebox=True):
    output_directory = ArchiveLib.GenerateFolderOutputPath(source_directory, ini_file_name)
    if output_directory == "":
        return
...
def CopyFolder(source_directory, ini_file_name, show_messagebox=True):
    output_directory = ArchiveLib.GenerateFolderOutputPath(
        source_directory, ini_file_name
    )
    if output_directory == "":
