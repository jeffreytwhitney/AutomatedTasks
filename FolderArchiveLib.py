...
def CopyFolderToSubDirectoryWithFileNameAppend(
    source_directory, sub_directory, ini_file_name
):
    ...
    messagebox.showinfo(
        "Success",
        f"Folder '{source_directory}' has been copied to '{output_subdirectory}'.",
    )
    return


def CopyFolderWithFileNameAppend(
    source_directory, ini_file_name, show_messagebox=True, recurse=False
):
    output_directory = ArchiveLib.GenerateFolderOutputPath(
        source_directory, ini_file_name
    )
    ...
    if show_messagebox:
        messagebox.showinfo(
            "Success",
            f"Folder '{source_directory}' has been copied to '{output_directory}'.",
        )
    return
...
def CopyFolderWithOverwrite(source_directory, ini_file_name):
    ...
    if os.path.exists(output_directory):
        user_response = messagebox.askyesno(
            "Are You Sure?",
            f"Directory '{output_directory}' already exists. Overwrite?",
        )
